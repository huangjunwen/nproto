...
func TestDurConn(t *testing.T) {
	...
	c := NewDurConn(nil, "test", OptReconnectWait(100*time.Millisecond))
	...
	// ### Subscribe: emulate normal subscribe.
	{
		tcfh.Go("QueueSubscribe#1", func() {
			c.QueueSubscribe("subj", "g1", nil, SubsOptResubscribeWait(100*time.Millisecond))
		})
	...
	// ### Subscribe before/after connection is lost.
	{
		tcfh.Go("QueueSubscribe#3", func() {
			c.QueueSubscribe("subj", "g3", nil, SubsOptResubscribeWait(100*time.Millisecond))
		})
		tcfh.Go("QueueSubscribe#4", func() {
			c.QueueSubscribe("subj", "g4", nil, SubsOptResubscribeWait(100*time.Millisecond))
		})
